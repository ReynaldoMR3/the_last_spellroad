...
- ~~What is the Mastery growth rate...~~ **Resolved 2026-07-25: 180 landed casts per tier (360 to fully master one spell).** Sized against Warden's real Level 1-3 wave data once it existed, per the developer's 2026-07-22 call to wait for it rather than guess. Full derivation, including a same-day self-caught correction (an initial 20-casts/tier pass wrongly assumed 1 landed cast ≈ 1 kill), lives in `docs/agents/_reference/mastery-template.md`; wired into `src/systems/MasterySystem.ts`. One related gap, found while verifying this number, is still open and tracked for Phase 5 rather than resolved here: `MasterySystem.recordLandedCast` fires on any landed hit, not a kill, so a player who deliberately keeps hitting one enemy without finishing it isn't bounded by level content at all — see `docs/agents/ana/backlog.md`, item 0.5.
