...
Every spell draws from a single Mana pool. Base pool size is 130 (retuned from 100, 2026-08-14, issue #235 — see below), and Mana regenerates passively at 8 per second at all times, in and out of combat (retuned from 5/sec 2026-08-03, backlog 2.34 / issue #77 — developer playtest found the opening minutes too Mana-starved for the intended "learn to cast fast" rush; see `mana-template.md` for the full retune derivation). This lets the mage cast a handful of spells back-to-back before needing to pause and let the pool recover, keeping the "read the battlefield, don't spam" fantasy intact without making the resource punishing.

**Wave 3 pacing retune (2026-08-14, issue #235, replacing closed #200):** a playtester still hit Mana trouble getting through wave 3 even after the wave-start Mana reset above. Root cause, independently re-derived: not the regen rate (already correct in steady state — single-spell spam never runs the pool dry) and not per-spell costs, but burst *depth* — reacting to a mixed-archetype wave with 2-3 different weight-class spells in quick succession (e.g. Light+Standard+Heavy = 65 Mana) drains far faster than 8/sec regen replaces it, and by wave 3's larger, more varied compositions (and worse, while a mana-regen-draining Debuffer is alive — see the Debuffer note above and issue #211), one burst could already leave the player pool-limited for the rest of the fight. Base pool raised 100 → 130 to buy room for two such bursts before the pool empties, without touching the already-validated regen rate, weight-class costs, or any wave's composition. Full derivation: `docs/agents/_reference/mana-template.md`, "Wave 3 Pacing Retune".
...
**Known limitation, accepted 2026-08-13 (issue #146):** because kill rewards (Mastery, Hexcoin) autosave permanently while enemy/wave microstate is *not* part of the saved blob, a player can quit to Title mid-level and Continue (resuming at the level's first wave) to replay already-cleared enemies for more rewards, without paying the death path's Mastery-tier-drop cost. This is an explicitly accepted limitation for the vertical slice, not an oversight — no save-format or economy rework is planned unless real playtesting shows it's actually being exploited or feels bad.
...
**Art direction without learning a map editor:** the developer-facing workflow is the local Art Board companion, not Tiled. It catalogs the repo's tiles, creatures, spell icons, VFX, maps, audio, and source/license records under stable IDs; the developer can browse those assets, make scene-level **Use**, **Replace**, or **Remove** decisions, and annotate intent such as a landmark or combat-lane constraint. The Art Board saves a versioned JSON art brief and produces a review-only proposal before any shipped file changes. This makes a visual request unambiguous to an implementation agent without requiring screenshot interpretation or individual tile painting. Initial descriptions/tags may be suggested by a frontier model, but canonical paths, source evidence, and human corrections remain authoritative; no local-model training or runtime model is part of the game.

**Getting sourced art into Phaser:** Tiled remains an internal compatibility/export format for shipped level geometry, not a required developer tool. Approved Art Board proposals are compiled into reviewable changes to the existing Tiled-format JSON maps or to explicit asset bindings, then loaded as usual with `this.load.image()` for spritesheets, `this.load.tilemapTiledJSON()` for layouts, `this.make.tilemap()`, `map.addTilesetImage()`, and `map.createLayer()`. This keeps the current Phaser pipeline intact while adding a fast, human-friendly direction layer above it.
