...
| 2.6 | Developer playtest: move, cast all 3 shapes, take damage from all 3 archetypes, die once, confirm Mastery-tier-drop and respawn-at-checkpoint both fire correctly | Developer | — | 2.3, 2.5 | `in-progress-with-owner` (owner: Loomwright) — first real developer session ran 2026-07-24 via Docker (`docker-compose up game`, real browser, not the sandboxed pane): moved, cast, killed 2 enemies, earned 2 Hexcoin. Found 1 BLOCKING crash (now fixed and re-verified, see 2.8) and 2 open UX/design gaps (2.9, 2.10). Death/respawn/Mastery-tier-drop logic itself is confirmed correct in code (`HealthSystem.applyDamage`/`handleDeath` both fire as designed) but couldn't be *observed* firing cleanly in-session because the crash interrupted the game immediately after. **Correction (2026-07-25):** re-attempting this does not need 2.9/2.10 resolved first — those are separate UX/design gaps (hit feedback, lane clamping/targeting input) unrelated to whether the death-trigger/respawn/Mastery-drop path fires correctly; gating the re-attempt on them was an unnecessary stall. Ready for a fresh developer session now that the crash fix is verified. |
...
| 2.9 | Design decision needed: no visible feedback when a spell hits (or fails to hit) an enemy | Ana (routes to developer) | — | 2.6 | `blocked-with-reason` — confirmed real, not a display bug hiding a working number: `Enemy.hp` (`src/entities/Enemy.ts`) is tracked but never rendered anywhere (no HP bar, no text, no color tint), and the only cast-feedback flash (`"Hit!"`) is wired to the *player* taking damage (`SpellroadScene.ts`'s `HealthSystem` constructor), not to landing a hit on an enemy. A non-lethal hit currently produces zero observable feedback, which is exactly what the developer reported ("my hp going down, but the monsters hp not"). Needs a developer call on the intended feedback (a per-enemy HP bar, a hit-flash/tint, a floating damage number, or some combination) before Loomwright builds it — this is new UI scope `engine-contract.md` doesn't currently cover. |
| 2.10 | Design decision needed: AoE shapes and enemies aren't constrained to the visible road lane, and targeting requires a mouse | Ana (routes to developer) | — | 2.6 | `blocked-with-reason` — two confirmed, related gaps. (1) The mage's physics body is explicitly clamped to the purple-bordered lane rectangle (`SpellroadScene.ts` `createMage`, `body.setBoundsRectangle(...)`), but `Enemy.ts` only calls Phaser's default `setCollideWorldBounds` (the full canvas, not the lane) — enemies can wander outside the lane. Separately, `SpellCaster.ts`'s shape geometry (`LINE_LENGTH` 220, `CONE_RADIUS` 180, `CIRCLE_RADIUS` 90) has no clamp to the lane rectangle at all (`ROAD_HEIGHT` is only 160px), so aiming even slightly off-horizontal visibly projects a cast outside the purple lines — matches the developer's "feels off" report exactly. (2) Targeting direction comes entirely from `this.input.activePointer` (`updatePreview`/`confirmCast`) with no non-mouse fallback, which the developer flagged as genuinely hard to use on a trackpad. Needs a developer call on the intended fix for each (clamp enemies/shapes to the lane; a non-mouse aiming default such as last-move-direction or nearest-enemy auto-target) before Loomwright implements — new scope beyond what `engine-contract.md` currently specifies. |
