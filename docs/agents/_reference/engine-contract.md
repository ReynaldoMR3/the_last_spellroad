...
**Scope extension, confirmed 2026-08-05 (code review flagged the drift; this doc had simply fallen behind actual practice, not a new decision):** Loomwright's engine scope also covers wiring already-authored/composed content into the gameplay scene's own audio and transient-UI surfaces — playing SFX one-shots and looped music via Phaser's sound manager (`sfx.ts`/`bgm.ts`), and displaying narrative/status banners already written by Lorena or the HUD's own status readouts. This was already established practice (backlog 3.10's SFX wiring, 2026-08-04) before this doc said so explicitly. Loomwright never authors the audio/prose content itself — only the engine-side call sites that play/display it.
