...
export class SpellroadScene extends Phaser.Scene {
  ...
  private spawnCastEffect(spell: SpellDefinition, targetX: number, targetY: number): void {
    ...
    // issue #111 — per-element base detune layered on top of the per-play variation, so a fire
    // cast and an ice cast no longer sound identical beyond their (already-distinct) visual tint.
    this.sound.play(sfxKey("cast"), computeSpellSfxVariation(spell.element));
